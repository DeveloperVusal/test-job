...
class MyModel extends Model
{
	...
	public function system_createUser($obj = [])
	{
		if (!preg_match("/^([a-z0-9_\.-]+)@([a-z0-9_\.-]+)\.([a-z\.]{2,60})$/", $obj['email'])) {
			return [
				'status' => 'error',
		...
		$sel2 = $this->handler->iSelectTable([
			'table-name' => 'users',
			'where' => [
				[
					'field' => 'email',
		...
		$hashPass = hash('sha256', CONFIG_SALT_PASSWORD.$obj['password'].CONFIG_SALT_PASSWORD);

		$ins = $this->handler->iInsertTable('users', [
			'login' => $obj['login'],
			'email' => $obj['email'],
			'password' => $hashPass,
	...
	public function system_userAuth($obj = [])
	{
		$hashPass = hash('sha256', CONFIG_SALT_PASSWORD.$obj['password'].CONFIG_SALT_PASSWORD);

		$resLogin = $this->handler->iSelectTable([
			'table-name' => 'users',
			'where' => [
				[
					'field' => 'login',
		...
		$resEmail = $this->handler->iSelectTable([
			'table-name' => 'users',
			'where' => [
				[
					'field' => 'email',
		...
		if ($resLogin->num_rows || $resEmail->num_rows) {
			$SESSIONS = [];
			$token = hash('sha256', $this->system_generateCode('chars', 80));
			$SESSIONS[] = [
				'user_agent' => $_SERVER['HTTP_USER_AGENT'],
				'token' => $token
			];

			$upd = $this->handler->iUpdateTable('users', [
				'sessions' => json_encode($SESSIONS)
			]);

			//setcookie('systemAuthToken', $token, time() + 604800, '/');
			$_SESSION['systemAuthToken'] = $token;
	...
	public function system_statusAuthUser()
	{
		if (empty($_COOKIE['systemAuthToken'])) {
			//$tokeen = hash('sha256', $this->system_generateCode('chars', 80));
			return false;
		} else {
			return true;
		}
	...
	public function system_mbStringToArray($string, $charset = 'UTF-8')
	{
	...
	public function system_getCountComments($arr = null)
	{
		$handler = new HandlerDB();
		$sel = $handler->iSelectTable([
			'table-name' => 'comments',
			'where' => [
				[
					'field' => 'basket',
		...
		return (int)$sel->fetch_row[0];
	}

	public function system_activityUser($userid, $type) {
		if ($type == 'set') {
			$handler = new HandlerDB();
			$_perfix = 'info_';

			$sel = $handler->iSelectTable([
				'table-name' => $_perfix.'users',
				'where' => [
					[
						'field' => 'user_id',
						'value' => $userid
			...
			if ($sel->num_rows) {
				$upd = $handler->iUpdateTable($_perfix.'users', [
					'last_activity' => time(),
					'basket' => 0
				], [[
					'user_id' => $userid
			...
			$sel2 = $handler->iSelectTable([
				...
				'select-fields' => ['last_activity']
			]);

			if ($sel2->num_rows) {
				$row = $sel2->fetch_assoc();
	...
	public function system_getUserStatus($array = null)
	{
		...
		$time = $this->system_activityUser($array['user_id'], 'get');
		
		if (!empty($time)) {
			if ($array['return'] == true) {
				return $this->system_showDate($time, $array['gender'], $array['limit']);
			} else {
				echo $this->system_showDate($time, $array['gender'], $array['limit']);
			}
		} else {
			if ($array['return'] == true) {
	...
	public function system_showDate($date, $gender = 0, $lim = 15)
	{
		$cur_d = getdate(time());
		$past_d = getdate($date);
		...
		$days    = array('сегодня', 'вчера');
		$month	 = array('', 'января', 'февраля', 'марта', 'апреля', 'мая', 'июня', 'июля', 'августа', 'сентебря', 'октября', 'ноября', 'декабря');
		$gender_word = array('был', 'была');
		
		if ($cur_d['mday'] !== $past_d['mday'] && $cur_d['mon'] == $past_d['mon'] && $cur_d['year'] == $past_d['year']) {
			$_dr = abs($cur_d['mday'] - $past_d['mday']);
			if ($_dr == 1) {
				if (strlen($past_d['minutes']) < 2) {
					$past_d['minutes'] = '0'.$past_d['minutes'];
				}
				return $gender_word[$gender].' '.$days[$_dr].' в '.$past_d['hours'].':'.$past_d['minutes'];
			} elseif ($_dr >= 2) {
				if (strlen($past_d['minutes']) < 2) {
					$past_d['minutes'] = '0'.$past_d['minutes'];
				}
				return $gender_word[$gender].' '.$past_d['mday'].' '.$month[$past_d['mon']].' в '.$past_d['hours'].':'.$past_d['minutes'];
			}
		} elseif ($cur_d['mday'] == $past_d['mday'] && $cur_d['mon'] == $past_d['mon'] && $cur_d['year'] == $past_d['year']) {
			$_horr=abs($cur_d['hours'] - $past_d['hours']);

			$cur_min = $cur_d['hours'] * 60 + $cur_d['minutes'];
			$past_min = $past_d['hours'] * 60 + $past_d['minutes'];
			$_minr = abs($cur_min - $past_min);

			if ($_horr == 0 && $_minr < $lim) {
				return 'Сейчас в сети';
			} elseif ($_minr >= $lim && $_minr < 60) {
				return $gender_word[$gender].' '.$_minr.' '.$minutes[2].' '.'назад';
			} elseif ($_minr >= 60) {
				if (strlen($past_d['minutes']) < 2) {
					$past_d['minutes'] = '0'.$past_d['minutes'];
				}
				return $gender_word[$gender].' '.$days[0].' в '.$past_d['hours'].':'.$past_d['minutes'];
			}
		} elseif ($cur_d['mon'] != $past_d['mon'] && $cur_d['year'] == $past_d['year']) {
			if (strlen($past_d['minutes']) < 2) {
				$past_d['minutes'] = '0'.$past_d['minutes'];
			}
			return $gender_word[$gender].' '.$past_d['mday'].' '.$month[$past_d['mon']].' в '.$past_d['hours'].':'.$past_d['minutes'];
		} elseif ($cur_d['year'] !== $past_d['year']) {
			if (strlen($past_d['minutes']) < 2) {
				$past_d['minutes'] = '0'.$past_d['minutes'];
			}
			return $gender_word[$gender].' '.$past_d['mday'].' '.$month[$past_d['mon']].' '.$past_d['year'].' в '.$past_d['hours'].':'.$past_d['minutes'];
		} else {
			return 'Сейчас в сети';
		}
	}

	public function system_base64_image($img)
	{
		$imageSize = getimagesize($img);
		$imageData = base64_encode(file_get_contents($img));
		$image = 'data:'.$imageSize['mime'].';base64,'.$imageData;
		return $image;
	}

	public function system_hashPassword($password)
	{
	...
	public function system_confirmPassword($hash, $password)
	{
	    return crypt($password, $hash) === $hash;
	}

	public function system_declensionWords($n, $s1, $s2, $s3)
	{
	    $m = $n % 10; $j = $n % 100;
	...
	public function system_reFormatCount($count)
	{
		/*
		 * 1 000 = 4
		 * 1 000 000 = 7
		...
		*/
		//$zc1 = 3;
		$word = '';
		$format = str_replace(' ', '', strval($count));

		if (strlen($count) >= 4 && strlen($count) <= 6) {
			$word = ' тыс.';
			$cut1 = substr($format, 0, strlen($count) - 3);
			$cut2 = (int)substr($format, -3);

			$cut2 = (int)substr(strval($cut2), 0, 1);
			...
			$format = $cut1.$cut2;
		} elseif (strlen($count) >= 7 && strlen($count) <= 9) {
			$word = ' млн.';
			$cut1 = substr($format, 0, strlen($count) - 6);
			$cut2 = (int)substr($format, -6);
			...
			$format = $cut1.$cut2;
		} elseif (strlen($count) >= 10 && strlen($count) <= 12) {
			$word = ' млрд.';
			$cut1 = substr($format, 0, strlen($count) - 9);
			$cut2 = (int)substr($format, -9);
	...
	public function system_htmlemoji($str)
	{
		...
		if (!function_exists('emoji_entity')) {
			function emoji_entity($matches) { return htmlspecialchars('&#'.hexdec(bin2hex(mb_convert_encoding("$matches[0]", 'UTF-32', 'UTF-8')))).';'; }
		}

		return preg_replace_callback($emoji_pattern, 'emoji_entity', $str);
	}

	public function system_realDateDiff($date1, $date2 = NULL)
	{
		/*
			Возращаемые данные
			0 - годы
	...
	public function system_declension_words($n, $s1, $s2, $s3)
	{
	    $m = $n % 10; $j = $n % 100;
	...
	public function system_reFormatDate($date)
	{
		$cur_d = getdate(time()); 
		$past_d = getdate($date);
		$diff = $this->system_realDateDiff(date("Y-m-d H:i:s",time()), date("Y-m-d H:i:s", $date));//Узнаем разность дат
		$seconds = array('секунды','секунд','секунду');
		$minutes = array('минуты','минут','минуту');
		$days    = array('сегодня','вчера');
		$moth	 = array('','января','февраля','марта','апреля','мая','июня','июля','августа','сентебря','октября','ноября','декабря');
		$genWord = array('добавил', 'добавила');

		if ($cur_d['mday'] !== $past_d['mday'] && $cur_d['mon'] == $past_d['mon'] && $cur_d['year'] == $past_d['year']) {
			$_dr = abs($cur_d['mday'] - $past_d['mday']);

			if ($_dr == 1) {
				if ($past_d['minutes'] < 10) {
					$past_d['minutes'] = "0".$past_d['minutes'];
				}

				return $genWord[$gender]." ".$days[$_dr]." в ".$past_d['hours'].":".$past_d['minutes'];
			} elseif ($_dr >= 2) {
				if ($past_d['minutes'] < 10) {
					$past_d['minutes'] = "0".$past_d['minutes'];
				}

				return $genWord[$gender]." ".$past_d['mday']." ".$moth[$past_d['mon']]." в ".$past_d['hours'].":".$past_d['minutes'];
			}
		} elseif ($cur_d['mday'] == $past_d['mday'] && $cur_d['mon'] == $past_d['mon'] && $cur_d['year'] == $past_d['year']) {
			$_minr = $diff[4];
			$_hour = $diff[3];
			...
			if ($_secr < 5 && $_minr < 1 && $_hour < 1) {
				return $genWord[$gender]." только что";
			} elseif ($_secr >= 5 && $_minr < 1 && $_hour < 1) {
				return $genWord[$gender]." ".$this->system_declension_words($_secr,$seconds[2],$seconds[0],$seconds[1])." назад";
			} elseif ($_minr >= 1 && $_minr < 60 && $_hour < 1) {
				return $genWord[$gender]." ".$this->system_declension_words($_minr,$minutes[2],$minutes[0],$minutes[1])." назад";
			} elseif ($_hour >= 1) {
				if ($past_d['minutes'] < 10) {
					$past_d['minutes'] = "0".$past_d['minutes'];
				}
				return $genWord[$gender]." ".$days[0]." в ".$past_d['hours'].":".$past_d['minutes'];
			}
		} elseif ($cur_d['mon'] != $past_d['mon'] && $cur_d['year'] == $past_d['year']) {
			if ($past_d['minutes'] < 10) {
				$past_d['minutes'] = "0".$past_d['minutes'];
			...
			return $genWord[$gender]." ".$past_d['mday']." ".$moth[$past_d['mon']]." ".$past_d['year']." в ".$past_d['hours'].":".$past_d['minutes'];
		}
	}

	public function system_userLikeDislikeFind($tableName, $tableID, $userID, $type = 1) {
		$handler = new HandlerDB();
		$sel = $handler->iSelectTable([
		...
		return ((int)$sel->num_rows)?true:false;
	}
}
?>